...
docker run -d -p 6379:6379 --name ${CONTAINER_NAME}_redis --restart unless-stopped redis

# Run app
# docker run --rm -d -p 8181:8181 --name ${CONTAINER_NAME} --link ${CONTAINER_NAME}_redis:redis -v /app ${CONTAINER_NAME}
docker run -d -p 8181:8181 --name ${CONTAINER_NAME} -v /app --restart unless-stopped ${CONTAINER_NAME}
